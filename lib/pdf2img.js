...
Pdf2Img.prototype.setOptions = function (opts) {
  ...
  options.maxPagesAtTime = opts.maxPagesAtTime || options.maxPagesAtTime;
};
...
Pdf2Img.prototype.convert = function (file, callbackreturn) {
  async.waterfall([
    ...
    function (pages, callback) {
      var processPage = function (page, callbackmap) {
      ...
      if (options.maxPagesAtTime === null) {
        return async.map(pages, processPage, callback);
      }
      return async.mapLimit(pages, options.maxPagesAtTime, processPage, callback);
    }
  ], callbackreturn);
};
