  ...
  async.waterfall([
      ...
      fs.stat(file, function (error, result) {
        if (error) callback('[Error: File not found]', null);
          ...
          if (!options.outputname) {
            options.outputname = path.basename(file, path.extname(path.basename(file)));
          }
          callback(null, file);
        }
      });
    ...
    function (pages, callback) {
      async.map(pages, function (page, callbackmap) {
        ...
        var outputStream = fs.createWriteStream(options.outputdir + '/' + options.outputname + '_' + page + '.' + options.type);
